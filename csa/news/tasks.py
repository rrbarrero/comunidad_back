...
from news.models import ComentarioNoticia
...
def notify_new_comment(comment_id):
    """Notifica cuando se publica un nuevo comentario en una noticia:
    ...
    try:
        comment = ComentarioNoticia.objects.get(pk=comment_id)
    except ComentarioNoticia.DoesNotExist:
        return
    recipients = []
    recipients.append(comment.noticia.autor)
    for cmnt in comment.noticia.comentarios.all():
    ...
    for user in list(set(recipients)):
        ...
        body = render_to_string(
            "mails/nuevo_comentario_noticia.html",
            {
                "username": user.username,
                "noticia_id": comment.noticia.id,
            },
        )
        send_mail(
            subject,
